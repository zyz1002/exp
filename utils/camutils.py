...
###新增添的方法
def get_image_cam(cam, cls_label, img_box=None, cfg=None):
    b, c, h, w = cam.shape
    # print(b, c, h, w)
    # pseudo_label = torch.zeros((b,h,w))
    cls_label_rep = cls_label.unsqueeze(-1).unsqueeze(-1).repeat([1, 1, h, w])
    valid_cam = cls_label_rep * cam
    # torch.max 返回两个tensor，第一个tensor表示对应维度的最大值；第二个tensor表示最大值的索引,一个对应值，一个对应类别
    cam_value, _pseudo_label = valid_cam.max(dim=1, keepdim=False)
    _pseudo_label += 1
    # 设置的超参bkg_score为0.45
...
def multi_scale_cam(model, inputs, scales):
    ...
    with torch.no_grad():
        ...
        _cam, _, _ = model(inputs_cat, cam_only=True)
...
def multi_scale_cam_with_aff_mat(model, inputs, scales):
    ...
    with torch.no_grad():
        inputs_cat = torch.cat([inputs, inputs.flip(-1)], dim=0)
        # 这个_aff_mat可以当作亲和度矩阵（也就是模型中的注意力矩阵）
        _cam, _aff_mat, _ = model(inputs_cat, cam_only=True)
        # print(_cam,_aff_mat)
...
def re_multi_scale_cam_with_aff_mat(model, inputs):
    cam_list, aff_mat = [], []
    b, c, h, w = inputs.shape
    with torch.no_grad():
        inputs_cat = torch.cat([inputs, inputs.flip(-1)], dim=0)

        _cam, _aff_mat, _x = model(inputs_cat, cam_only=True)
        _cam = F.relu(_cam)
        _cam = _cam / (F.adaptive_max_pool2d(_cam, (1, 1)) + 1e-5)
        recam_feature = _cam.unsqueeze(2) * _x.unsqueeze(1)  # bs*512*20*32*32
        recam_feature = recam_feature.view(recam_feature.size(0), recam_feature.size(1), recam_feature.size(2), -1)
        recam_feature = torch.mean(recam_feature, -1)  # bs*
        recam_feature = recam_feature.permute(0, 2, 1)
        # print(_cam,_aff_mat)
        # aff_mat.append(_aff_mat)
    return recam_feature, _cam, _x
    # _cam = F.interpolate(_cam, size=(h, w), mode='bilinear', align_corners=False)
    # _cam = torch.max(_cam[:b, ...], _cam[b:, ...].flip(-1))
