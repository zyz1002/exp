...
class VOC12ClsDataset(VOC12Dataset):
    def __init__(self,
        ...
        self.local_view = T.Compose([
            ...
            self.flip_and_color_jitter,  #color jiter 是数据增强的一种，颜色抖动是指对图像的曝光度（exposure）、饱和度（saturation）和色调（hue）
            # 进行随机变化形成不同光照及颜色下的图片，达到数据增强的目的，尽可能使得模型能够使用不同光照条件小的情形，提高模型泛化能力
